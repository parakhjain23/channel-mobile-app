...
export const RenderTextWithLinks = React.memo(
  ({
    text,
    repliedContainer,
    orgState,
    ...
    const userIdAndDisplayNameMapping = useMemo(
    ...
    const findKeyByValue = useCallback(
      value => {
        ...
        for (let key in userIdAndDisplayNameMapping) {
          if (userIdAndDisplayNameMapping[key] === newValue) {
            return key;
          }
        }
        return null;
    ...
    function highlight(text, result, repliedContainer) {
      const textColor = repliedContainer
        ? 'black'
        : sentByMe
        ? colors.sentByMeTextColor
        : colors?.textColor;
